...
export const Tag = ({ tagName, tagColor }: TagProps) => {
  return (
    <span style={tagColor} className="rounded-full border border-gray-600 px-3 py-1 text-xs">
      {tagName}
    </span>
  );
};
...
export const TagStatus = ({ type }: { type: 'todo' | 'doing' | 'done' }) => {
  const statusInfo = {
    todo: {
      text: '未着手',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="size-3.5 sm:size-4" width="32" height="32" viewBox="0 0 24 24">
          <path
            fill="currentColor"
  ...
  return (
    <span
      className={clsx(
        'flex items-center gap-2 rounded-full text-xs font-bold',
        'py-1 pr-2.5 pl-1 sm:pr-4 sm:pl-2',
